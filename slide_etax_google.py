    ...
    def run(self):
        ...
        while failure:
            ...
            try:
                ...
                try:
                    self.wait.until(
                    EC.text_to_be_present_in_element((By.CLASS_NAME, 'yidun_tips__text'), '失败过多，点击重试'))
                    print(f'too many times...')
                    retry = self.driver.find_element_by_class_name('yidun_tips__text')
                    retry.click()
                except:
                    print('success!')
                    return
